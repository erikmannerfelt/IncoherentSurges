...
def rec_update_dict(dictionary: dict, new: dict, i: int = 0, max_i: int = 20):
    ...
    for key, value in new.items():
        if isinstance(value, collections.abc.Mapping):
            dictionary[key] = rec_update_dict(dictionary.get(key, {}), value, i=i + 1, max_i=max_i)
        else:
            dictionary[key] = value
    return dictionary

def rec_to_tex_macros(dictionary: dict, pre_key: str = "", i: int = 0, max_i: int = 20) -> list[str]:
    if i == max_i:
        raise RecursionError("Max recursion limit reached")
...
def surge_statistics(force_redo: bool = False):
    ...
    for key, data in [("bottom_up", bottom_up_stats), ("top_down", top_down_stats), ("all", combined_stats)]:
    ...
    for key in ["terminus", "instability"]:
        fact = out_info["advance_rates"][key]["top_down"] / out_info["advance_rates"][key]["bottom_up"]
        ...
        for row_n, row in enumerate(glaciers):
            for col_n, glacier in enumerate(row):
                ...
                if not pd.isna(surge_start := stats.loc[glacier, "surge_start"]):
                    ...
                    data["date_diff_to_start"] = np.abs((data.index.get_level_values("date") - surge_start).total_seconds().values) / (3600 * 24)

                    closest_lower = data.loc["lower_coh"].sort_values("date_diff_to_start").iloc[0]
                ...
                if surge_termination != xlim[1] or surge_start != xlim[0]:
                    ...
                    surge_ylim = (
                        max(lims.get("surge_start_coh", ylim)[0], lims.get("surge_termination_coh", ylim)[0]),
                        min(lims.get("surge_start_coh", ylim)[1], lims.get("surge_termination_coh", ylim)[1])
